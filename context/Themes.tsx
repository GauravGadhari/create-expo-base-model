...
export const ThemeDynamicProvider: React.FC<ThemeDynamicProviderProps> = ({ children }) => {
    ...
    const [dynamicTheme, setDynamicTheme] = useState<Theme>(colorScheme === 'light' ? lightBlue : darkBlue);

    useEffect(() => {
        setDynamicTheme(colorScheme === 'light' ? lightBlue : darkBlue);
    }, [colorScheme]);

    return (
        <ThemeDynamicContext.Provider value={{ theme: dynamicTheme }}>
            {children}
